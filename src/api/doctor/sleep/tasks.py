from celery.schedules import crontab
from celery.task import periodic_task

from doctor.sleep import constants
from doctor import constants as general_constants
from doctor.sleep.models import SleepDailyAssessment
from doctor.sleep.utils import (
    check_required_sleep_completion
)


@periodic_task(run_every=(crontab(general_constants.CRON_30_MINUTES)), name=constants.DAILY_SLEEP_ASSESSMENTS)
def task_check_and_create_daily_sleep_assessment(**kwargs):
    """
        Task to check and create daily sleep assessment tasks
        Runs every 30 minutes
    """
    try:
        if not kwargs.get('test'):
            previous_sleep_assessment_objects = SleepDailyAssessment.objects.in_progress().\
                with_user_current_date_time().for_previous_day().with_user_midnight_time()
        else:
            previous_sleep_assessment_objects = SleepDailyAssessment.objects.in_progress().\
                with_user_current_date_time().for_previous_day()
        for previous_sleep_assessment_object in previous_sleep_assessment_objects:
            try:
                check_required_sleep_completion(
                    previous_sleep_assessment_object,
                    previous_sleep_assessment_object.sleep_type
                )
            except Exception as e:
                print(
                    general_constants.FAILED_NOTIFICATION_USER_MESSAGE.format(
                        constants.DAILY_SLEEP_ASSESSMENTS,
                        previous_sleep_assessment_object.user.id,
                        str(e)
                    )
                )
        if not kwargs.get('test'):
            print(
                general_constants.SUCCESS_NOTIFICATION_MESSAGE.format(
                    constants.DAILY_SLEEP_ASSESSMENTS
                )
            )
    except Exception as e:
        print(
            general_constants.FAILED_NOTIFICATION_MESSAGE.format(
                constants.DAILY_SLEEP_ASSESSMENTS,
                str(e)
            )
        )
